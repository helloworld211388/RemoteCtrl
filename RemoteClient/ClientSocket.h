...
{
public:
	static CClientSocket* getInstance() {//静态函数  还是public  在类外面可以调用（语法上面的规定）
		if (m_instance == NULL) {  //静态函数没有this 指针，所以无法直接访问成员变量
...
#define BUFFER_SIZE 4096000 //把409600 改为 4096000  结果成功啦
	int DealCommand() {
	...
	//bool SendPacket(const CPacket& pack, std::list<CPacket>& lstPackets, bool isAutoClosed = true);
